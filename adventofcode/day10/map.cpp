...
struct custLocation {
  size_t x, y;
  double distance;
  custLocation(size_t _x, size_t _y, double _distance) : x{_x}, y{_y}, distance{_distance} {};
...
int main() {
  ...
  size_t maxDetects = 0;
  custLocation maxDetectLocation;
  // quadrant numbers:
  //     0
  // 7 2 | 1  1
  // 6 -----  2
  ...
  for (custLocation &currentCoords : asteroids) {
    for (custLocation &potentialDetection : asteroids) {
      ...
      // (y2 - y1) / (x2 - x1)
      double dy =
          (double)potentialDetection.y - (double)currentCoords.y;
      double dx =
          (double)potentialDetection.x - (double)currentCoords.x;
      double slope;
      ...
      } else if (potentialDetection.distance < detectedAsteroids[theslope].distance) {
        detectedAsteroids[theslope] = potentialDetection;
      }
    }
    if (detectedAsteroids.size() > maxDetects) {
      maxDetects = detectedAsteroids.size();
      maxDetectLocation = currentCoords;
      maxDetectedAsteroids = detectedAsteroids;
    }
    detectedAsteroids.clear();
  }
  cout << "found " << maxDetects << " asteroids at (" << maxDetectLocation.x
       << ',' << maxDetectLocation.y << ')' << endl;
}
